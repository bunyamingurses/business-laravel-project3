...
class indexController extends Controller
{
    ...
    public function carousel()
    {
        ...
        return View("admin.setting.carousel",["carousel"=>$carousel]);
    }


    public function carouselPost(Request $request)
    {
        $all=[];

        if ($request->File("foto") != null) {
            $images = functionController::imageCreate($request,"carousel","foto");
            $imagesAvif = functionController::imageCreateAvif($request,"carousel","foto");
            $all+=[
                "image"=>$images,
                "imageAvif"=>$imagesAvif,
            ];
        }
        ...
        if ($all!=null && $all!=""){
            ...
            if ($create){
                return redirect()->back()->with("status","Carousel Eklendi.");
            }else{
                return redirect()->back()->with("status","Carousel Eklenemedi.");
            }
        }
    }
    ...
    {
        if (isset($id)){
            $carousel=carousel::find(functionController::security($id));
            if (isset($carousel->id)){
                $carousel->delete();
                return redirect()->back()->with("status","Carousel Silindi.");
            }else{
                return redirect(route("admin.index"));
            }
        }
